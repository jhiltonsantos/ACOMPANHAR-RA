...
public class DraggableObjects2Nivel4 : MonoBehaviour, IDraggableObjects2Nivel4
{
    private ARRaycastManager raycastManager;
    private List<ARRaycastHit> hits = new List<ARRaycastHit>();
    public bool isBeingDragged;
    private Rigidbody rb;
    private DestinationObjects2Nivel4 destinoScript;
    public string correctTagDestination;

    void Start()
    {
        raycastManager = FindObjectOfType<ARRaycastManager>();
        rb = GetComponent<Rigidbody>();
        destinoScript = GetComponentInParent<DestinationObjects2Nivel4>();
    }
    ...
    {
        if (other.CompareTag(correctTagDestination))
        {
